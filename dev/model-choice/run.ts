...
const root = fs.mkdtempSync(path.join(os.tmpdir(), "lerience-model-choice-"));
...
async function run(providerId: "claude" | "codex"): Promise<void> {
  ...
  fs.writeFileSync(
    ...
    "Synthetic model-choice acceptance fixture. For any session opener, reply exactly MODEL_CHOICE_OK. Do not use tools, read files, or perform course work.",
  );
  ...
  const conductor = new SessionConductor({
    ...
    emitSnapshot: (snapshot) => log({ providerId, snapshot }),
  });
...
void app.whenReady().then(async () => {
  fs.writeFileSync(path.resolve("out/model-choice-location.txt"), root);
